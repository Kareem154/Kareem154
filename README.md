<h1 align="center">Hi, I'm Karim Mohamed 👋</h1> <p align="center">
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&pause=1000&color=00C2FF&center=true&vCenter=true&width=700&lines=Backend+Developer;Laravel+Expert;PHP+Architect;Package+Author;Always+Innovating" />
</p>

[![Portfolio](https://img.shields.io/badge/PORTFOLIO-VISIT-blue?style=for-the-badge&logo=git&logoColor=white)](https://github.com/Kareem154)[![GitHub](https://img.shields.io/badge/GITHUB-KAREEM154-black?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Kareem154)[![LinkedIn](https://img.shields.io/badge/LINKEDIN-KAREEM-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/kareem-mohamed-578189249/)[![Email](https://img.shields.io/badge/EMAIL-CONTACT-red?style=for-the-badge&logo=gmail&logoColor=white)](mailto:karim.mohamed.dev@gmail.com)

---

### 👋 About Me

I am a dedicated **Backend Developer** with extensive expertise in building scalable, secure, and maintainable web applications using **PHP & Laravel**. I focus on clean architecture, robust database design, and high-performance backend systems. Passionate about AI integration and solving complex business logic challenges.

---

### 🏆 Highlights

- **Package Author:** Created and published the [Smart Backup](https://packagist.org/packages/karim-dev/smart-backup) Laravel package.

- **AI Specialist:** Expert in integrating AI assistants with local knowledge bases to prevent hallucinations.

- **Backend Architect:** Deep experience in designing RESTful APIs, authentication flows, and service-based code organization.

- **Production Ready:** Skilled in cPanel/VPS deployment, debugging, and performance optimization.

---

### 💼 Experience

**Backend Developer***Freelance & Contract* | Present

- Developing high-performance **RESTful APIs** and complex backend systems.

- Implementing **AI-driven solutions** within Laravel applications for enhanced user interaction.

- Managing database migrations, optimization, and secure file upload architectures.

**Laravel Package Developer***Open Source* | 2026

- Developed **Smart Backup**, a package for streamlined database and storage management.

- Focused on providing a clean API and easy integration for other Laravel developers.

---

### 💻 Skills

#### 🚀 Backend

![PHP](https://img.shields.io/badge/-PHP-777BB4?style=flat&logo=php&logoColor=white) ![Laravel](https://img.shields.io/badge/-Laravel-FF2D20?style=flat&logo=laravel&logoColor=white) ![MySQL](https://img.shields.io/badge/-MySQL-4479A1?style=flat&logo=mysql&logoColor=white) ![Redis](https://img.shields.io/badge/-Redis-DC382D?style=flat&logo=redis&logoColor=white) ![REST APIs](https://img.shields.io/badge/-REST_APIs-007ACC?style=flat&logo=rest&logoColor=white) 

#### 🛠️ Tools & Platforms

![Git](https://img.shields.io/badge/-Git-F05032?style=flat&logo=git&logoColor=white) ![Docker](https://img.shields.io/badge/-Docker-2496ED?style=flat&logo=docker&logoColor=white) ![Postman](https://img.shields.io/badge/-Postman-FF6C37?style=flat&logo=postman&logoColor=white) ![cPanel](https://img.shields.io/badge/-cPanel-FF6C2C?style=flat&logo=cpanel&logoColor=white) 

---

### 🚀 Projects

| Project Name | Description | Tech Stack | Status |
| --- | --- | --- | --- |
| Smart Backup | A professional Laravel package for database/storage management. | PHP, Laravel, Packagist | Published |
| AI Knowledge Assistant | AI-driven assistant integrated into Laravel with local data trust. | Laravel, OpenAI API, MySQL | Active |
| Admin Dashboard | Scalable management system with roles and permissions. | Laravel, Blade, MySQL | Production |

---

### GitHub Stats

<p align="center">
<a href="https://github.com/Kareem154">
    <img src="https://github-readme-stats.vercel.app/api?username=Kareem154&show_icons=true&theme=dark&hide_border=true&count_private=true" alt="Kareem154's GitHub Stats" />
  </a>
  <a href="https://github.com/Kareem154">
    <img src="https://github-readme-streak-stats.herokuapp.com/?user=Kareem154&theme=dark&hide_border=true" alt="Kareem154's GitHub Streak" />
  </a>
  <a href="https://github.com/Kareem154">
    <img src="https://github-profile-trophy.vercel.app/?username=Kareem154&theme=dark&no-frame=true" alt="Kareem154's GitHub Trophies" />
  </a>
  <a href="https://github.com/Kareem154">
    <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Kareem154&layout=compact&theme=dark&hide_border=true" alt="Kareem154's Top Languages" />
  </a>
</p>
